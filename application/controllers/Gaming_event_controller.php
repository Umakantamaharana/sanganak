...
class Gaming_event_controller extends CI_Controller {
...
';
    ...
    public function event(){
		...
		//	$this->form_validation->set_rules('name5', 'Name', 'required');
			$this->form_validation->set_rules('event_name', 'Name', 'required|callback_game_event_check');
            ...
            if($this->form_validation->run() == true){
                $insert = $this->User_model->game_insert($userData);
                if($insert){
...
';
         ...
         if($this->input->post('Submit3')){
            ...
            if($this->form_validation->run() == true){
                $insert = $this->User_model->game_insert($userevent);
                if($insert){
                    echo '<script>alert("You Have Successfully Registered For This Event!");</script>';
                    $url = 'gaming_event_controller/event';
echo'
...
';
        ...
        //load the view
        $this->load->view('game_event_view', $data);
    }else{
